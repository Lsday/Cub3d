...
/*   Updated: 2022/02/19 10:53:49 by oronda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*read_file(int fd, char *savedbuffer, char *buff)
{
	...
	while (!has_newline(savedbuffer) && r > 0)
	{
		...
		{
			buff[r] = '\0';
			savedbuffer = ft_gnl_strjoin(savedbuffer, buff);
		}
	}
	if (*savedbuffer == '\0')
	{
...
char	*cut_before_newline(char *buffer)
{
	...
	while (buffer[i] != '\n' && buffer[i])
		i++;
	result = ft_gnl_substr(buffer, 0, i + 1);
	return (result);
}
...
char	*cut_after_newline(char *buffer)
{
	...
	while (buffer[i] != '\n' && buffer[i])
		i++;
	strlen = ft_gnl_strlen(buffer);
	result = ft_gnl_substr(buffer, i + 1, strlen - i);
	free(buffer);
	return (result);
}
